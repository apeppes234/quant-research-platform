...
export function BiasLedger() {
  ...
  return (
    <Card
      ...
      data-testid="bias-ledger"
      aria-label="Bias ledger"
    >
      <CardHeader className="phase-panel__header">
        <div>
          <CardTitle className="panel-title">Bias Ledger</CardTitle>
          <CardDescription className="panel-subtitle">
            {ledger.length} variants logged
          </CardDescription>
        </div>
        <CardAction className={summary.dsr > 0 ? "dsr dsr--pass" : "dsr"}>
          <span>DSR</span>
          <Badge variant={summary.dsr > 0 ? "default" : "destructive"}>
            {formatNumber(summary.dsr)}
          </Badge>
        </CardAction>
      </CardHeader>

      <CardContent className="phase-panel__content ledger-panel__content">
        ...
        <ScrollArea className="ledger-list">
          <div className="ledger-list__inner">
            {ledger.length === 0 ? (
              <div className="phase-empty">No variants logged yet.</div>
            ) : null}
            {ledger
              .slice(-5)
              .reverse()
              .map((entry) => (
                <div
                  key={entry.variant_id ?? `${entry.path}:${entry.ts}`}
                  className="ledger-row"
                >
                  <div>
                    <strong>
                      {entry.variant_id || entry.kind || "variant"}
                    </strong>
                    <span>
                      {entry.ts
                  ...
                  <Badge variant="outline">{formatGap(entry)}</Badge>
                </div>
              ))}
          </div>
        </ScrollArea>
...
function summarizeLedger(entries: LedgerEntry[]) {
  ...
  const explicit = [...entries]
    .reverse()
    .find(
      (entry) => typeof entry.deflated_sharpe_ratio === "number",
    )?.deflated_sharpe_ratio;
  const bestHoldout = sharpes.length ? Math.max(...sharpes) : 0;
  const dsr = explicit ?? computeDeflatedSharpe(sharpes, trials);
  const gaps = entries
    .map((entry) =>
      typeof entry.in_sample_sharpe === "number" &&
      typeof entry.holdout_sharpe === "number"
